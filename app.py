...
def handle_message(event):
    ...
    if re.match("想知道股價[0-9]" , msg):
        ...
        line_bot_api.push_message(TextSendMessage("in2"))#
        return 0
    #新增使用者關注的股票到mongodb
    if re.match('關注[0-9]{4}[0-9]',msg):
        stockNumber = msg[2:6]
        line_bot_api.push_message(TextSendMessage("in"))#
    ...
    if (msg.startswith('#')):
        text = msg[1:]
        content =''
        line_bot_api.push_message(TextSendMessage("in3"))#
    
    if (emsg.startswith('#')):
        text = emsg[1:]
        content =''
        line_bot_api.push_message(TextSendMessage("in4"))#
